...
export default function TranslationHistory() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.translation.list.useQuery(undefined, { enabled: isAuthenticated });
  ...
  const handleSignedDownload = async (
    ...
    try {
      const { url } = await utils.translation.download.fetch({
        id: translationId,
      });
      ...
      else toast.error("فشل الحصول على رابط التنزيل");
    }
  };
  ...
  const filteredTranslations = useMemo(() => {
    ...
    return translations.filter(translation => {
      if (filterStatus !== "all" && translation.status !== filterStatus)
        return false;
      if (q === "") return true;
      const src = findLanguage(translation.sourceLanguage);
      const tgt = findLanguage(translation.targetLanguage);
      const haystack = [
                  ...
                  <div className="grid md:grid-cols-5 gap-4 items-center">
                    ...
                    <div className="md:col-span-1">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <span
                          className="px-2 py-1 bg-card rounded text-xs font-semibold"
                          title={renderLanguage(translation.sourceLanguage)}
                        >
                          {translation.sourceLanguage.toUpperCase()}
                        </span>
                        <span className="text-muted-foreground">←</span>
                        <span
                          className="px-2 py-1 bg-accent/10 rounded text-xs font-semibold text-accent"
                          title={renderLanguage(translation.targetLanguage)}
                        >
